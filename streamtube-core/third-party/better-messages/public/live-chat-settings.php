...
/**
 *
 * The live chat edit post settings template file
 * 
...
<form class="form-ajax form-add-post" method="post">
    <div class="widget widget-live-chat-settings">
        ...
        <div class="widget-content">
			...
			<?php 
            if( Streamtube_Core_Permission::moderate_posts() && $post_id ){
                echo '<input type="hidden" name="bpbm[allow_guests]">';              
                $better_messages->admin->settings_template( get_post( $post_id ) );
            }else{

                $settings = $better_messages->admin->get_settings( $post_id );
		    ...
		    <?php printf(
		        ...
		        $post_id && comments_open( $post_id ) ? 'open' : 'closed'
		    );?>
