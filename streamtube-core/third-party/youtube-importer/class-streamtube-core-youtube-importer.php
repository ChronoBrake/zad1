...
class StreamTube_Core_Youtube_Importer{
     ...
     * 
     */
    public function is_existed( $yt_id ){
        global $wpdb;

        $sql = "
            SELECT * FROM $wpdb->postmeta AS meta
            INNER JOIN $wpdb->posts AS posts ON posts.ID = meta.post_id
            WHERE posts.post_type = 'video' AND meta_value LIKE '%s'
        ";

        $results = $wpdb->query( $wpdb->prepare( $sql, "%{$yt_id}%" ) );
    ...
    public function import_content( $yt_video_ids, $importer_id = 0 ){
        ...
        if( $response['items'] && count( $response['items'] ) > 0 ){
            for ( $i=0; $i < count( $response['items'] ); $i++) {
                ...
                if( ! $this->is_existed( $this->api->video->get_item_id( $response['items'][$i] ) ) ){
                    ...
                    if( ! is_wp_error( $post_id ) ){
                        ...
                        if( $settings['post_tags'] ){
                            if( $tags = $this->api->video->get_item_tags( $response['items'][$i] ) ){
                         ...
                         * 
                         */
                        do_action( 'streamtube/core/youtube_importer/imported', $post_id, $importer_id, $response['items'][$i], $response );
