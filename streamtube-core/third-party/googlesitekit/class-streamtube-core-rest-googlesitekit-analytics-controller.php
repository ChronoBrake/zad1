...
class StreamTube_Core_GoogleSiteKit_Analytics_Rest_Controller extends StreamTube_Core_GoogleSiteKit_Rest_Controller{
    ...
    public function get_default_path(){
        ...
        $video_object = get_post_type_object( Streamtube_Core_Post::CPT_VIDEO );

        if( get_option( 'permalink_structure' ) ){
            $default_path = trailingslashit( $parsed_url['path'] ) . $video_object->rewrite['slug'];
        }else{
            $default_path = $parsed_url['path'] . '?' . $video_object->rewrite['slug'];
        }

        return apply_filters( 'streamtube/core/rest/analytics/default_path', $default_path );
    }
    ...
    public function get_user_own_page_paths( $limit = -1 ){
        ...
        foreach ( $posts as $post ) {
            $page_paths[] = get_permalink( $post->ID );
        }
        ...
        if( $request['page_path'] ){
            ...
            if( $this->analytics->can_moderate() ){
                ...
                $params['dimensionFilter']['andGroup']['expressions'][] = array(
                    'filter'    =>  array(
                        'inListFilter'  =>  array(
                            'values' =>  $this->get_user_own_page_paths()
                        ),
